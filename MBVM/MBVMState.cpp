...
void MBVMStateAvailable::Run(MBVMContext& context, int event_id, int value) {
    switch(event_id) {
        case EventID::POWER:
            ...
            break;
        case EventID::BUY:
            if(context.SelectItem(static_cast<CoffeeType>(value))) {
                context.SetStatus(MBVMStatus::MAKING_COFFEE);
//                context.SupplyItem(static_cast<CoffeeType>(value));
            }
            break;
        case EventID::PAY:
            context.Pay(static_cast<MoneyType>(value));
            break;
        case EventID::CANCEL:
            context.Cancel();
            break;
